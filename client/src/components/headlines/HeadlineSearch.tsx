    ...
    <div className="w-full max-w-md">
      <div className="relative group">
        <Input
          ...
          icon={<Search size={14} className="transition-colors duration-fast group-focus-within:text-yellow" />}
          trailing={
            value ? (
              <button
                onClick={clear}
        ...
        {value && (
          <div className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-yellow/20 rounded-full animate-ping pointer-events-none" />
        )}
      </div>
    </div>
  );
