      ...
      <div className="max-w-md w-full text-center">
        {status === 'loading' && (
          <div className="flex flex-col items-center gap-4">
            <Spinner size="lg" />
            <p className="font-mono text-sm text-muted">Unsubscribing...</p>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center gap-4 animate-fade-in">
            ...
            <p className="font-mono text-sm text-secondary leading-relaxed">
              You've been removed from the Feed Frenzy newsletter.<br />
              We're sad to see you go, but we get it.
            </p>
            <Link
              to="/newsletter"
              className="font-mono text-xs text-yellow hover:underline mt-4"
            >
        ...
        {status === 'already' && (
          <div className="flex flex-col items-center gap-4 animate-fade-in">
            <CheckCircle size={64} className="text-yellow" />
            <h1 className="font-display text-3xl tracking-widest text-yellow">
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center gap-4 animate-fade-in">
            <XCircle size={64} className="text-coral" />
            <h1 className="font-display text-3xl tracking-widest text-coral">
              INVALID LINK
            </h1>
            <p className="font-mono text-sm text-secondary leading-relaxed">
            ...
            {errorMessage && (
              <p className="font-mono text-xs text-muted bg-surface-2 px-3 py-2 rounded-sm">
                Error: {errorMessage}
              </p>
            )}
            <Link
